    ...
    <!-- FOTO + DETAIL  -->
     ...
     <!-- COMMNET SECTION -->
      <div class="my-4 py-4 border-y border-y-gray-300 ">
        <div :class="`h-[200px] lg:h-[calc(100vh-600px)] overflow-hidden hover:overflow-y-scroll ${postComment.length < 1 ? 'flex justify-center items-center ' : ''} `" ref="commentBlock" >
          <Skeleton :loading="isCommentLoading" active/>
          <div class="h-max w-full" v-if="postComment.length > 0 && !isCommentLoading">
...
<Modal
  ...
  >
    <div class="">
      ...
      <!-- @vue-ignore -->
     <LazyDetailLeafletMap :latitude="postDetail?.coordinate.latitude" :longitude="postDetail?.coordinate.longitude" :location-name="postDetail?.coordinate.locationName" />
    </div>
  </Modal>
