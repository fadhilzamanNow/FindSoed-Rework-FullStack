...
import { computed, h, onMounted, reactive, ref, toRaw, useTemplateRef, watchEffect } from 'vue';
...
onMounted(() => {
    if(mapRef.value){
        myMap.value = leaflet.map(mapRef.value,{zoomControl : false}).setView([mapInfo?.latitude ? mapInfo?.latitude : -7.428846146037751 , mapInfo?.longitude ? mapInfo?.longitude : 109.34005811432314],10)
    ...
    myMarker.value = leaflet.marker([mapInfo.latitude ? mapInfo.latitude :  -7.397339842404089, mapInfo.longitude ? mapInfo.longitude : 109.35116941228598], {icon : customMarker, draggable : false}).addTo(myMap.value as Map)
...
const inputMapProps = computed<InputProps>(() => ({
    placeholder : mapInfo.locationName ? mapInfo.locationName : mapInfo.latitude && mapInfo.longitude ? `${mapInfo.latitude},${mapInfo.longitude}` : "Cari Lokasi",
    onInput : (e : ChangeEvent) => handleSearch((e.target as HTMLInputElement).value),
...
watchEffect(() => {
    console.log("map Inf : ", mapInfo.locationName ? mapInfo.locationName : mapInfo.latitude && mapInfo.longitude ? `${mapInfo.latitude},${mapInfo.longitude}` : "Cari Lokasi")
})
    ...
    <div class="flex  flex-col items-center justify-center  gap-6  w-full " >
        <div  class="flex flex-col gap-4 sm:border sm:max-w-[640px] sm:border-gray-200 w-full justify-center  rounded-md sm:p-4">
            ...
            <Flex vertical class="w-full " gap="8" >
                <label for="item" @click="() => console.log('file list : ',fileList)" >Nama Barang</label>
            ...
            <Flex vertical class="w-full " gap="8">
                ...
                <DatePicker placement="bottomRight" @change="(_,e) => itemDate = e" format="YYYY-MM-DD"/>
            </Flex>
            <Flex vertical class="w-full" gap="8">
                <label for="item">Foto Barang</label>
                <div class="h-[10vh] overflow-auto">
                    <Upload v-model:file-list="fileList" list-type="picture" :before-upload="beforeUpload" @remove="handleRemove" :default-file-list="fileList" accept=".png,.jpeg,.jpg" >
                        <Button>Upload Gambar</Button> 
                    </Upload>
                </div>
            </Flex>
            <Flex vertical class="w-full" gap="8">
                <label for="item" @click="() => handleReset()">Lokasi Kehilangan</label>
                <div class="w-full h-[20vh] relative">
                    <div class="absolute z-[2] top-0 left-0 mt-1 ml-2 ">
                        <div class="w-[250px] md:w-[350px]">
                            <div class="relative h-max">
                                <div>
                                    <Input.Search  class="relative z-[2]"  enter-button v-bind="inputMapProps" />
                                </div>
                                <div class="max-h-[10vh] bg-white rounded-md p-2 flex flex-col overflow-auto text-xs" v-if="searchMap.length > 0 && showSearhResult" >
                                    <div v-if="searchResult" >
                                        <div v-if="searchResult.length > 0">
                                            <div v-for="(v,i) of searchResult" class="hover:bg-slate-100 bg-white transition-all duration-100 ease-in p-1 rounded-md cursor-pointer" :key="i" @click="handleChooseMap(v)" >
                                                {{ v.formatted }}
                                            </div>
                                        </div>
                                        <div v-else>
